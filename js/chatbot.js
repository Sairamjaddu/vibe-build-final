(function() {
  ...
  function scoreIntent(messageLower) {
    const scores = {};
    const CONFIDENCE_THRESHOLD = 2; // Minimum score to confidently assign an intent
    const WORD_BOUNDARY_BOOST = 1.5; // Boost for exact word matches
    const START_OF_SENTENCE_BOOST = 2; // Boost for terms at the beginning of the message
    ...
    // Calculate scores with improved matching logic
    for (const key in INTENTS) {
      for (const term of INTENTS[key]) {
        const termLower = term.toLowerCase();
        
        // Check for start of sentence matches (higher priority)
        const startRegex = new RegExp('^' + termLower, 'i');
        if (startRegex.test(messageLower)) {
          scores[key] += START_OF_SENTENCE_BOOST * (term.length >= 4 ? 2 : 1);
          continue;
        }
        ...
        if (wordBoundaryRegex.test(messageLower)) {
          scores[key] += WORD_BOUNDARY_BOOST * (term.length >= 4 ? 2 : 1);
          continue;
        }
        
        // Fallback to partial matches but with lower weight
        if (messageLower.includes(termLower)) {
          scores[key] += term.length >= 4 ? 1 : 0.5;
        }
      }
    }
    
    // Boost for questions (as before but more targeted)
    if (/how|\bwhat|\bwhen|\bwhere|\bwhy|\bwhich|\bwho/.test(messageLower)) {
      for (const key in scores) scores[key] += 0.5;
    }
    
    // Find the best intent with confidence
    let best = 'other';
    let bestScore = 0;
    ...
    // If no intent meets the confidence threshold, use 'other'
